...
static sljit_s32 atomicWaitCallback(ExecutionContext* context, sljit_s32 memOffset, sljit_s32 size)
{
    ...
    if (size == 8) {
        instance->memory(0)->atomicWait(context->state, instance->module()->store(), memOffset, 0, expect, timeout, &result);
    } else {
        instance->memory(0)->atomicWait(context->state, instance->module()->store(), memOffset, 0, (int32_t)expect, timeout, &result);
    }
...
static void emitAtomicNotify(sljit_compiler* compiler, Instruction* instr)
{
    ...
    sljit_emit_op1(compiler, SLJIT_MOV32, SLJIT_R1, 0, addr.baseReg, 0);
    sljit_emit_op1(compiler, SLJIT_MOV, SLJIT_R0, 0, kContextReg, 0);
